...
const tagController = {
    ...
    associateTagToCard: async (req, res) => {
        try {
            ...
            // D'ou vient dette méthode addTag ??
            // Doc sequelize : https://sequelize.org/master/manual/assocs.html#special-methods-mixins-added-to-instances
            // plus d'explications sur cette issue : https://github.com/O-clock-Cheesecake/Soutien-ateliers/issues/57
    ...
    removeTagFromCard: async (req, res) => {
        try {
            ...
            await card.removeTag(tag); // idem :removeTag est une méthode spéciale ajoutée par Sequelize pour gérer les associations
            card = await Card.findByPk(cardId, {
                include: ['tags']
            });
            res.json(card);
